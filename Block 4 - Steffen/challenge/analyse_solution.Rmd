...
## install.packages(c("rmarkdown", "tidyverse", "ggthemes", "svglite", "lubridate", "cowplot", "rgeos", "rgdal", "maptools", "leaflet"))
library(tidyverse)
library(ggthemes)
library(lubridate)
library(cowplot)
...
bundeslaender_geo_raw <- readOGR("data/bundeslaender.geo.json", verbose = FALSE)
...
### Choroplethen-Karte mit Symbolen
...
### Choroplethen-Karte mit Leaflet

```{r}
pal <- colorNumeric("viridis", NULL)

leaflet(bundeslaender_geo_raw) %>%
  addTiles() %>%
  addPolygons(
    stroke = FALSE,
    smoothFactor = 0.3,
    ...
    fillColor = ~ function (x)
      bundeslaender_daten %>% filter(Bundesland == x)),
    
    label = ~paste0(NAME_1, ": ", ID_1)
  ) %>%
...
leaflet(staedte_daten) %>%
  ...
  addCircleMarkers(
    lat = ~Lat,
    lng = ~Long, 
    radius = ~log(Teilnehmer),
    label = ~paste0(Ort, ": ", Anzahl, " Demos, ", Teilnehmer, " Teilnehmer"),
    stroke = FALSE,
    color = "#DF6233",
    fillOpacity = 0.25
    # color = ~pal(type),
...
```{r}
leaflet() %>%
  setView(lat = 48.781647, lng = 11.414274, zoom = 13) %>% 
  addProviderTiles(
    "CartoDB.Positron",
    options = providerTileOptions(noWrap = TRUE)
  ) %>%
  ...
  addRectangles(
    lng1=48.781647, lat1=11.414274,
    lng2=48.581647, lat2=11.314274,
    stroke = "green",
    color = "green",
    fillOpacity = 0.2
  )
